...
BEGIN_MESSAGE_MAP(CMainFrame, CFrameWndEx)
	...
	ON_MESSAGE(Theme::WM_SUBTAB_CHANGED, &CMainFrame::OnSubTabChanged)
END_MESSAGE_MAP()
...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	// Dashboard toolbar — second top-docked row, Dashboard-tab only
	m_wndDashToolbar.Create(_T(""), this, CRect(0, 0, 0, Theme::DASH_TOOLBAR_H), FALSE,
		ID_DASHTOOLBAR, WS_CHILD | WS_VISIBLE | CBRS_TOP);
	m_wndDashToolbar.EnableGripper(FALSE);
	m_wndDashToolbar.EnableDocking(CBRS_ALIGN_TOP);
	DockPane(&m_wndDashToolbar, AFX_IDW_DOCKBAR_TOP);

	// Leftmost column — high-resolution inspection image viewer. Docked first
	// so it becomes the outermost-left column; the panes below stack to its
	// right in call order (all still freely user-draggable afterward).
	m_wndImagePane.Create(_T("Inspection Image"), this, CRect(0, 0, 380, 600), TRUE,
		ID_PANE_IMAGE,
		WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | WS_CLIPCHILDREN | CBRS_LEFT | CBRS_FLOAT_MULTI);
	m_wndImagePane.EnableDocking(CBRS_ALIGN_LEFT | CBRS_ALIGN_RIGHT | CBRS_ALIGN_BOTTOM);
	DockPane(&m_wndImagePane, AFX_IDW_DOCKBAR_LEFT);

	// Middle dashboard column — Panel List -> Defect Summary -> Stats -> Yield Chart
	m_wndPanelListPane.Create(_T("Panel List"), this, CRect(0, 0, 220, 220), TRUE,
		ID_PANE_PANELLIST,
		WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | WS_CLIPCHILDREN | CBRS_LEFT | CBRS_FLOAT_MULTI);
	m_wndPanelListPane.EnableDocking(CBRS_ALIGN_LEFT | CBRS_ALIGN_RIGHT | CBRS_ALIGN_BOTTOM);
	DockPane(&m_wndPanelListPane, AFX_IDW_DOCKBAR_LEFT);

	m_wndDefectSummaryPane.Create(_T("Defect Summary"), this, CRect(0, 0, 190, 110), TRUE,
		ID_PANE_DEFECTSUMMARY,
		WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | WS_CLIPCHILDREN | CBRS_LEFT | CBRS_FLOAT_MULTI);
	m_wndDefectSummaryPane.EnableDocking(CBRS_ALIGN_LEFT | CBRS_ALIGN_RIGHT | CBRS_ALIGN_BOTTOM);
	DockPane(&m_wndDefectSummaryPane, AFX_IDW_DOCKBAR_LEFT);

	m_wndStatsPane.Create(_T("Statistics"), this, CRect(0, 0, 170, 130), TRUE,
		ID_PANE_STATS,
		WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | WS_CLIPCHILDREN | CBRS_LEFT | CBRS_FLOAT_MULTI);
	m_wndStatsPane.EnableDocking(CBRS_ALIGN_LEFT | CBRS_ALIGN_RIGHT | CBRS_ALIGN_BOTTOM);
	DockPane(&m_wndStatsPane, AFX_IDW_DOCKBAR_LEFT);

	m_wndYieldChartPane.Create(_T("Today Yield Data"), this, CRect(0, 0, 220, 180), TRUE,
		ID_PANE_YIELD,
		WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | WS_CLIPCHILDREN | CBRS_LEFT | CBRS_FLOAT_MULTI);
	m_wndYieldChartPane.EnableDocking(CBRS_ALIGN_LEFT | CBRS_ALIGN_RIGHT | CBRS_ALIGN_BOTTOM);
	DockPane(&m_wndYieldChartPane, AFX_IDW_DOCKBAR_LEFT);
...
{
	m_navIndex = (int)wParam;
	ShowView(m_navIndex);
	UpdatePaneVisibility();
	return 0;
}

LRESULT CMainFrame::OnSubTabChanged(WPARAM wParam, LPARAM /*lParam*/)
{
	m_dashboardSubTab = (int)wParam;
	UpdatePaneVisibility();
	return 0;
}
...
//   Dashboard-body panes: Monitoring view AND its "Main View" sub-tab only
//   Dashboard toolbar   : Monitoring view, any sub-tab
//   Model pane          : Recipe view only
// ---------------------------------------------------------------------------

void CMainFrame::UpdatePaneVisibility()
{
	bool onDashboard = (m_navIndex == Theme::VIEW_DASHBOARD);
	bool showBody    = onDashboard && (m_dashboardSubTab == Theme::SUBTAB_MAIN);

	m_wndDashToolbar.ShowPane(onDashboard, FALSE, FALSE);
	m_wndImagePane.ShowPane(showBody, FALSE, FALSE);
	m_wndPanelListPane.ShowPane(showBody, FALSE, FALSE);
	m_wndDefectSummaryPane.ShowPane(showBody, FALSE, FALSE);
	m_wndStatsPane.ShowPane(showBody, FALSE, FALSE);
	m_wndYieldChartPane.ShowPane(showBody, FALSE, FALSE);
	m_wndDefectPane.ShowPane(showBody, FALSE, FALSE);
	m_wndModelPane.ShowPane(m_navIndex == Theme::VIEW_RECIPE, FALSE, FALSE);
}
...
{
	if (m_navIndex < (int)m_moduleViews.size() && m_moduleViews[m_navIndex] && m_moduleViews[m_navIndex]->m_hWnd)
		m_moduleViews[m_navIndex]->SetFocus();
	else
		m_wndView.SetFocus();
}
...
{
	if (m_navIndex < (int)m_moduleViews.size() && m_moduleViews[m_navIndex])
		if (m_moduleViews[m_navIndex]->OnCmdMsg(nID, nCode, pExtra, pHandlerInfo))
			return TRUE;
