...
struct DefectInfo
{
    ...
    TCHAR         thumbnailPath[MAX_PATH]; // cropped defect image on disk; empty = none
    double        confidence;        // 0..100, model confidence ("po%" column)
    TCHAR         lotId[32];
    TCHAR         boardId[32];
    __time64_t    timestamp;         // detection time, UTC
};
